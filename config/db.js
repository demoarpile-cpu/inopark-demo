...
const runAutoMigrations = async () => {
  try {
    ...
    const [estimatesExist] = await pool.execute(
      `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES 
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'estimates'`
    );

    if (estimatesExist.length > 0) {
    ...
    // CRM Pipelines & Lead Sources (crm_db_innopark extensions)
    try {
      const [leadSourcesTable] = await pool.execute(
        `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'lead_sources'`
      );
      if (leadSourcesTable.length === 0) {
        await pool.execute(`
        ...
        `);
        console.log('✅ lead_sources table created');
      }
      const [leadPipelinesTable] = await pool.execute(
        `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'lead_pipelines'`
      );
      if (leadPipelinesTable.length === 0) {
        await pool.execute(`
        ...
        `);
        console.log('✅ lead_pipelines & lead_pipeline_stages tables created');
      }
      const [dealPipelinesTable] = await pool.execute(
        `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'deal_pipelines'`
      );
      if (dealPipelinesTable.length === 0) {
        await pool.execute(`
        ...
        `);
        console.log('✅ deal_pipelines & deal_pipeline_stages tables created');
      }
      const [dealContactsTable] = await pool.execute(
        `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'deal_contacts'`
      );
      if (dealContactsTable.length === 0) {
        await pool.execute(`
      ...
      // Add columns to leads, deals, activities, companies if missing
      const leadCols = ['pipeline_id', 'stage_id', 'lead_source_id'];
      for (const col of leadCols) {
        ...
        if (c.length === 0) {
          await pool.execute(`ALTER TABLE leads ADD COLUMN ${col} INT UNSIGNED NULL`);
          console.log(`✅ leads.${col} added`);
        }
      }
      const dealCols = ['pipeline_id', 'stage_id', 'contact_id'];
      for (const col of dealCols) {
        const [c] = await pool.execute(
          `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'deals' AND COLUMN_NAME = ?`,
          [col]
        );
        if (c.length === 0) {
          await pool.execute(`ALTER TABLE deals ADD COLUMN ${col} INT UNSIGNED NULL`);
          console.log(`✅ deals.${col} added`);
        }
      }
      const activityCols = ['is_pinned', 'follow_up_at', 'meeting_link'];
      for (const col of activityCols) {
        const [c] = await pool.execute(
          `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'activities' AND COLUMN_NAME = ?`,
          [col]
        );
        if (c.length === 0) {
          const def = col === 'is_pinned' ? 'TINYINT(1) DEFAULT 0' : col === 'follow_up_at' ? 'DATETIME NULL' : 'VARCHAR(500) NULL';
          await pool.execute(`ALTER TABLE activities ADD COLUMN ${col} ${def}`);
          console.log(`✅ activities.${col} added`);
        }
      }
      const companyCols = ['delivery_address', 'object_address'];
      for (const col of companyCols) {
        const [c] = await pool.execute(
          `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'companies' AND COLUMN_NAME = ?`,
          [col]
        );
        if (c.length === 0) {
          await pool.execute(`ALTER TABLE companies ADD COLUMN ${col} TEXT NULL`);
          console.log(`✅ companies.${col} added`);
        }
      }
    } catch (crmMigErr) {
      console.log('⚠️ CRM pipeline migration (non-fatal):', crmMigErr.message);
    }
